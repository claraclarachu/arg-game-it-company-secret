...
`).filter(s=>s.trim()!=="");if(!t.length)return{header:[],rows:[]};const a=s=>{const n=[];let i="",r=!1;for(let d=0;d<s.length;d++){const o=s[d];o==='"'?r&&s[d+1]==='"'?(i+='"',d++):r=!r:o===","&&!r?(n.push(i),i=""):i+=o}return n.push(i),n};return{header:a(t[0]),rows:t.slice(1).map(a)}}function Js(e,t){const{header:a,rows:s}=Gs(t),n=`<thead><tr>${a.map(r=>`<th>${h(r)}</th>`).join("")}</tr></thead>`,i=`<tbody>${s.map((r,d)=>`<tr>${r.map(o=>`<td>${h(o)}</td>`).join("")}${r.length<a.length?`<td colspan="${a.length-r.length}"></td>`:""}</tr>`).join("")}</tbody>`;return`
  ...
  `}function ce(e){if(Se(e)){const i=document.getElementById("vsEditor");i&&(i.innerHTML='<div class="editor__lines" style="padding:16px;color:var(--fg-muted)">檔案不存在或尚未解鎖 — 嘗試 Search 搜尋 "Sawyer" 或觸發隱藏邏輯</div>');return}if(e===me){N=e,Ne(),Ke(),Oe(document.getElementById("vsQuickOpen")?.value.trim().toLowerCase()||"");return}N=e,Ne();const t=S.getFile(e),a=da(e),s=document.getElementById("vsEditor");if(!s)return;if((e==="/customer-portal/src/main/java/com/nori/OrderService.java"||e==="/customer-portal/src/billing/service.js")&&Kn("vscode_viewed"),a==null)s.innerHTML='<div class="editor__lines" style="padding:16px;color:var(--fg-muted)">檔案不存在或尚未解鎖 — 嘗試 Search 搜尋 "Sawyer" 或觸發隱藏邏輯</div>';else if(e.toLowerCase().endsWith(".csv"))s.innerHTML=Js(e,a);else{const i=t?.meta?.lang||(e.endsWith(".py")?"python":e.endsWith(".java")?"java":"javascript");s.innerHTML=`
...
//# sourceMappingURL=main-B7yyP_5S.js.map